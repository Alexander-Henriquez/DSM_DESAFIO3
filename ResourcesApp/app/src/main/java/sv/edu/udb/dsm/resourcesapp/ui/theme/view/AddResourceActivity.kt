...
class AddResourceActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        saveButton.setOnClickListener {
            ...
            if (resourceId == null) {
                // Crear nuevo recurso
                val newResource = Resource(0, title, description, link, type) // Usa 0 o un nuevo ID aquí
                createResource(newResource)
            } else {
                // Editar recurso existente
                val updatedResource = Resource(resourceId!!, title, description, link, type) // Usar resourceId
    ...
    private fun loadResourceData(id: Int) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val resource = RetrofitClient.api.getResourceById(id.toString()) // Asegúrate que este método acepte un Int
    ...
    private fun updateResource(resource: Resource) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                RetrofitClient.api.updateResource(resource.id, resource)
                withContext(Dispatchers.Main) {
